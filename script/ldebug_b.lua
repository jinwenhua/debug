...
function l_debug:_set_enable(info, benable)
	...
	if info.enable ~= benable then
		if benable == 0 then
			...
			if info.enable ~= benable and self.enable_count <= 0 and dgethook() then
				self.enable_count = 0;
				self:unhook();
			end
		else
...
function l_debug:is_set_line_hook(s_source, n_linedefined, n_lastlinedefined)
...
function l_debug.hook_c(cmd)
    ...
    -- self.fwrite(sinfo, self.WRITE_INFOR);
	self:is_set_line_hook(s_source, n_linedefined, n_lastlinedefined);
end
...
function l_debug.hook_crl(cmd, line)
	...
	elseif cmd == "call" and self.mode == self.DEBUG_MODE_STEP then
        self:set_mode(self.DEBUG_MODE_NEXT);
		self.set_in_count = self.set_in_count + 1;
		return;
	elseif cmd == "return" and self.set_in_count > 0 then
		self.set_in_count = self.set_in_count - 1;
		return;
	elseif cmd == "return" and self.set_in_count <= 0 then
		self.set_in_count = 0;
